...
def _grab_pixmap_qt():
    ...
    if screen is None:
        return False, "Не удалось получить доступ к экрану."
    pixmap = screen.grabWindow(0)
    if pixmap.isNull():
        return False, "Не удалось выполнить захват экрана."
    return True, pixmap
...
def _grab_pixmap_wayland():
    ...
    if not grim:
        return False, (
            "Захват экрана под Wayland требует утилиту 'grim'. "
            "Установите пакет 'grim' (и 'slurp' для выбора области) "
    ...
    try:
        ...
        if image.isNull():
            return False, "Не удалось прочитать снимок, сделанный grim."
        return True, image
    except Exception as e:
        return False, f"Ошибка вызова grim: {e}"
    finally:
...
def save_pixmap(pixmap, cfg):
    ...
    try:
        ...
        return False, f"Не удалось создать папку сохранения: {e}"
    ...
    if not ok:
        return False, "Не удалось сохранить файл скриншота."
    return True, path
...
def copy_pixmap_to_clipboard(pixmap):
    ...
    if clipboard is None:
        return False, "Буфер обмена недоступен в этом окружении."
...
def _grab_region_wayland():
    ...
    try:
        ...
        return "error", f"Ошибка вызова slurp: {e}"
    ...
    try:
        ...
        image = QImage(tmp_path)
        if image.isNull():
            return "error", "Не удалось прочитать снимок выделенной области."
    except Exception as e:
        return "error", f"Ошибка вызова grim: {e}"
    finally:
        if os.path.isfile(tmp_path):
            try:
                os.remove(tmp_path)

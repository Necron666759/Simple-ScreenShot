...
class TrayApp(QObject):
    ...
    def __init__(self, app):
        ...
        self._menu_actions = {}  # action_key -> QAction, для retranslate_ui()
        self._build_menu()
    ...
    def _build_menu(self):
        ...
        capture_action = QAction(tr("tray.capture_now"), menu)
        capture_action.triggered.connect(self.capture_now)
        menu.addAction(capture_action)

        clipboard_action = QAction(tr("tray.capture_clipboard"), menu)
        clipboard_action.triggered.connect(self.capture_to_clipboard)
        menu.addAction(clipboard_action)

        region_action = QAction(tr("tray.capture_region"), menu)
        ...
        self._menu_actions = {
            "capture_now": capture_action,
            "capture_clipboard": clipboard_action,
            "capture_region": region_action,
    ...
    def retranslate_ui(self):
        """Обновляет тексты трея (подсказку и меню) сразу после смены
        языка в диалоге настроек, без пересоздания самого трея."""
        self.tray.setToolTip(tr("app_name"))
        self._menu_actions["capture_now"].setText(tr("tray.capture_now"))
        self._menu_actions["capture_clipboard"].setText(tr("tray.capture_clipboard"))
        self._menu_actions["capture_region"].setText(tr("tray.capture_region"))
        self._menu_actions["settings"].setText(tr("tray.settings"))
        self._menu_actions["quit"].setText(tr("tray.quit"))
    ...
    def capture_now(self):
        ...
        if ok:
            self.tray.showMessage(
                tr("app_name"), tr("notify.saved", path=result),
                QSystemTrayIcon.Information, 3000,
            )
        else:
            self.tray.showMessage(
                tr("app_name"), tr("notify.error", error=result),
    ...
    def capture_to_clipboard(self):
        ...
        if ok:
            self.tray.showMessage(
                tr("app_name"), tr("notify.clipboard_saved"),
                QSystemTrayIcon.Information, 2500,
            )
        else:
            self.tray.showMessage(
                tr("app_name"), tr("notify.error", error=error),
                QSystemTrayIcon.Warning, 5000,
            )
    ...
    def capture_region(self):
        ...
        if result is None:
            self.tray.showMessage(
                tr("app_name"), tr("notify.region_clipboard_saved"),
                QSystemTrayIcon.Information, 2500,
            )
        else:
            self.tray.showMessage(
                tr("app_name"), tr("notify.region_saved", path=result),
                QSystemTrayIcon.Information, 3000,
            )
    ...
    def open_settings(self):
        if self.settings_dialog_instance is not None:
            ...
            return
        dlg = SettingsDialog(self.cfg, self._apply_new_config, on_language_change=self.retranslate_ui)
